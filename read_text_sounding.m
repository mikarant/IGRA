...
% StationID can be seen for example from the map shown here:
...
for i=1:1000000
    % read header
    header = fgetl(fileID);
    % if end-of-file is encountered, break the loop
    if header==-1
        break
    end
    % year 
    ...
    if ind500>0
        % if there are two records from the same pressure level, choose the
        % first one
        z500=z(ind500(1)); 
